...
function EditCategoryModal({ questionObject, setEditModal, editQuestion, categories }: {questionObject: Question, setEditModal: Function, editQuestion: Function, categories: Array<Category>}) {
    ...
    const [answers, setAnswers] = useState([questionObject.answers[0].content, questionObject.answers[1].content, questionObject.answers[2].content, questionObject.answers[3].content]);
    const [category, setCategory] = useState(questionObject.category);
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        answers.forEach((val, index) => {
            const answer = { id: questionObject.answers[index].id, content: val}
